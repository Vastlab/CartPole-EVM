...
class CartPoleSwingUp(gym.Env):
    ...
    def __init__(self):
        self.gravity = 9.8000000
        self.masscart = 1.0
        self.masspole = 0.100000
    ...
    def get_best_action(self, observation):
        ...
        observation_right, reward, done, info = self.step(1)
        right_future = self.cartpole_cost_numpy(observation_right, cost_sigma=0.25)

        observation = self.set_state(current_state)

        if right_future < left_future:
            action = 1
            expected_state = observation_right
        else:
            action = 0
            expected_state = observation_left
        return action, expected_state

    def cartpole_cost_numpy(self, state, cost_sigma=0.25):
        cart_x = state[0]
        theta = state[2]
        pole_x = self.length*np.sin(theta)
        pole_y = self.length*np.cos(theta)

        tip_position = pole_y = self.length*np.cos(theta)
        target = np.array([self.length])
